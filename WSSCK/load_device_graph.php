...
foreach($result as $row)
{
    if($type==0)
    {  //kvar data
        $kvar1 = round($row['cval1']*$row['val1']/1000* sin(acos($row['pf1'])),2);
        $kvar2 = round($row['cval2']*$row['val2']/1000* sin(acos($row['pf2'])),2);
    ...
    elseif($type==2)
    { // kva data
        $kva1 = round($row['cval1']*$row['val1']/1000,2);
        $kva2 = round($row['cval2']*$row['val2']/1000,2);
        $kva3 = round($row['cval3']*$row['val3']/1000,2);
        $graphName = "KVA Graph";
    ...
    elseif($type==3)
    {   // peak and off peak data
        $val1 = round($row['val1'],2);
        $val2 =round($row['val2'],2);
        $val3 =round($row['val3'],2);
    ...
    elseif($type==4)
    {   // peak and off peak data
        $cval1 = round($row['cval1'],2);
        $cval2 =round($row['cval2'],2);
        $cval3 =round($row['cval3'],2);
