...
require_once("opendb.php");
	$id = $_GET['id'];
	$query = "select * from billing_postpaid where bill_id = '$id'";
...
<form method = "POST">
		...
		<div class = "form-group row">
			...
			<div class="col-sm-6">                                        
					<input type="text" class="form-control" value="<?php echo $row['cid']; ?>" disabled />                                  
			</div>
		</div>
		...
		<div class = "form-group row">
				...
				<div class="col-sm-6">                                        
						<input type="text" class="form-control" name = "amount" placeholder = "Enter Amount Collected" required = "required" />                                  
				</div>
		</div>
...
<?php
	...
	if ($stmt -> rowCount() > 0) {
		...
		$result = $stmt->execute();
		$count = $stmt->rowCount();
		echo "<script>window.location.href = 'ebill.php?id=$cid'</script>";
	}
