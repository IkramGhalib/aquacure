...
$q = "SELECT * from db_current_logs where dbid = '".$id."' order by id desc limit 20";
$result= $conn -> query($q) or die("Query error");                          
foreach($result as $row)
{
    if($type==0)
    {  //kvar data
        $kvar1 = round($row['line1_c1']*$row['v1']/1000* sin(acos($row['line1_pf1'])),2);
        $kvar2 = round($row['line1_c2']*$row['v2']/1000* sin(acos($row['line1_pf2'])),2);
        $kvar3 = round($row['line1_c3']*$row['v3']/1000* sin(acos($row['line1_pf3'])),2);
    ...
    elseif($type==2)
    { // kva data
        $kva1 = round($row['line1_c1']*$row['v1']/1000,2);
        $kva2 = round($row['line1_c2']*$row['v2']/1000,2);
        $kva3 = round($row['line1_c3']*$row['v3']/1000,2);
        $graphName = "KVA Graph";
    ...
    elseif($type==4)
    {   // peak and off peak data
        $cval1 = round($row['line1_c1'],2);
        $cval2 =round($row['line1_c2'],2);
        $cval3 =round($row['line1_c3'],2);
