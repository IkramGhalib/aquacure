...
$q = "select peak, offpeak, (peak+offpeak) as kwh, datetime from tr_kwh_logs where trid = '$id' and datetime in (select max(datetime) from tr_kwh_logs where trid ='$id' and datetime > DATE_SUB(now(), INTERVAL 30 DAYS)group by day(datetime))";
...
foreach($result as $row)
{
    ...
    for ($i=0; $i < sizeof($data1)-1; $i++) { 
        array_push($temp1, $data1[$i+1] - $data1[$i]);
        array_push($temp2, $data2[$i+1] - $data2[$i]);
        array_push($temp3, $data3[$i+1] - $data3[$i]);
    }
    $data1 = $temp1;
    $data2 = $temp2;
    $data3 = $temp3;
