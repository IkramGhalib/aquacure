...
require_once("opendb.php"); 
  $q = "SELECT kwh, date from tbl_kwh where trid = '".$id."' and date > NOW()- INTERVAL 5 DAY";
// $q = "select peak, offpeak, date(datetime) as datetime from tr_kwh_logs where trid = '$id' and datetime in (select max(datetime) from tr_kwh_logs where trid ='$id' and datetime >  NOW()- INTERVAL 30 DAY group by day(datetime))";
// echo $q;
$result= $conn -> query($q) or die("Query error");                          
foreach($result as $row)
{
        $peak = 0;
        $offpeak = round($row['kwh'],2);
        $total = $peak + $offpeak;
        $graphName = "Total Consumption Graph (kWh)";

        array_push($data1,$peak);
        array_push($data2,$offpeak);
        array_push($data3,$total);
        array_push($data4,$row['date']);                                                
}
$data5 = $data3;
