                ...
                <div class="col-md-2">
                  ...
                  <datalist id="pumps" >
                    <?php
                      ...
                      $query = "Select trid, name, location from transformer where flowmeter = 1";
                      $result = $conn -> query($query) or die(error);

                      foreach ($result as $row) {
                        ?>
                        <option value="<?php echo $row['trid'];?>"><?php echo $row['location']." - ".$row['location']; ?></option>
                        <?php
                      }
                    ?>
    ...
    require_once("opendb.php");
          ...
          if(isset($_POST['transformer'])){
            ...
            if(empty($_POST['todate']) or empty($_POST['fromdate'])){
              $query = "SELECT fm_logs.*, transformer.name, transformer.location, transformer.zone, transformer.uc, transformer.nc, transformer.twid FROM fm_logs ,transformer WHERE fm_logs.pump_id = transformer.trid and fm_logs.pump_id = '$id' and fm_logs.instant_flow > 0 order by fm_logs.datetime desc limit 100";
              // echo "check1";
              ?>
            ...
            else
            {
             ...
             // echo "check 2";
              ...
              $query = "SELECT fm_logs.*, transformer.name, transformer.location, transformer.zone, transformer.uc, transformer.nc, transformer.twid FROM fm_logs ,transformer WHERE fm_logs.pump_id = transformer.trid and fm_logs.pump_id = '".$id."' and fm_logs.datetime BETWEEN '".$fromdate."' AND '".$todate."' and fm_logs.instant_flow > 0 order by fm_logs.datetime";
              ?>
