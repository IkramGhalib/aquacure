    ...
    <section class="content">
      ...
      elseif ($_SESSION['billing_method'] == 'prepaid') {
        ...
        foreach ($result as $row) {
          ...
          if($row['slot1']%3 == 1){
            $volt = $row['v1'];
          }elseif($row['slot1']%3 == 2){
            $volt = $row['v2'];
          }elseif($row['slot1']%3 == 0){
            $volt = $row['v3'];
          }

          $v1 = $volt;
          $pf1 = $row['pf1'];
          $kva1 = $row['c1'] * $volt/1000;
        }

        $query = "SELECT SUBSTRING(status, ".$conn_no.",1) as st, switch_time".$conn_no." as swtime FROM `db_status` WHERE dbid = '".$split_cid[0]."' ";
          $result = $conn->query($query) or die(error);
          // echo $query;
          foreach($result as $row){
            $status = ($row['st'] == 0) ? 'Off' : 'On';
            $switch_time = $row['swtime'];
          }
