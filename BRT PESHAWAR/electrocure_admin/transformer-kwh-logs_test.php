       ...
       <?php
          ...
          if(isset($_POST['transformer'])){
            ...
            if(empty($_POST['todate']) or empty($_POST['fromdate'])){
              $query = "SELECT FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(tr_kwh_logs.datetime) / (15*60)) * (15*60)) AS dt,name, tr_kwh_logs.trid, max(tr_kwh_logs.peak_dev) as peak_dev,max(tr_kwh_logs.offpeak_dev) offpeak_dev from tr_kwh_logs, transformer where transformer.trid = tr_kwh_logs.trid and tr_kwh_logs.trid = '".$id."' GROUP BY tr_kwh_logs.trid, dt, name ORDER BY dt desc limit 500";
            ...
            else
            {
              ...
              if(!(empty($_POST['fromtime']) or empty($_POST['totime']))){
               ...
               $query = "SELECT FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(tr_kwh_logs.datetime) / (15*60)) * (15*60)) AS dt,name, tr_kwh_logs.trid, max(tr_kwh_logs.peak_dev) as peak_dev,max(tr_kwh_logs.offpeak_dev) offpeak_dev from tr_kwh_logs, transformer where transformer.trid = tr_kwh_logs.trid and tr_kwh_logs.trid = '".$id."' AND tr_kwh_logs.datetime BETWEEN '".$fromdate."' AND '".$todate."' GROUP BY tr_kwh_logs.trid, dt, name ORDER BY dt desc";
          ...
          $query= "SELECT FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(tr_kwh_logs.datetime) / (15*60)) * (15*60)) AS dt,name, tr_kwh_logs.trid, max(tr_kwh_logs.peak_dev) as peak_dev,max(tr_kwh_logs.offpeak_dev) offpeak_dev from tr_kwh_logs, transformer where transformer.trid = tr_kwh_logs.trid GROUP BY tr_kwh_logs.trid, dt, name ORDER BY dt desc limit 500";
          }
