...
<head>
  ...
  <?php $pageName = "Change ElectroCure Tariff"?>
...
<form method = "POST">
		...
		<div class = "form-group row">
				<label class="col-sm-2 col-form-label">FPA</label>
				<div class="col-sm-6">                                        
						<input type="text" class="form-control" name = "fpa" placeholder = "Fuel Price Adjustment" required = "required" />                                  
				</div>
		</div>
...
<?php
	...
	$q = "insert into tariff_electrocure (prepaid_unit,postpaid_unit,fpa,gst,surcharge) values('$prepaid','$postpaid','$fpa','$gst','$surcharge')";
	echo $q;
	$stmt = $conn->prepare($q);
	try{
	$result = $stmt->execute();
	$count = $stmt->rowCount();
	echo "<script>window.location.href = 'tariff_electrocure.php'</script>";
	}
