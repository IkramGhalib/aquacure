    ...
    <section class="content" id="sec">
      ...
      if ($detect->isMobile() or $detect->isTablet()) {
        ...
        ?>
          ...
          <button class="btn <?php  echo ($phase == 1) ? 'btn-danger' : 'btn-primary' ; ?>" onclick="window.location.href = 'connection-current-logs.php?phase=1'">Single Phase</button>
          <button class="btn <?php  echo ($phase == 3) ? 'btn-danger' : 'btn-primary' ; ?>" onclick="window.location.href = 'connection-current-logs.php?phase=3'">Three Phase</button><br><br>
    ...
    <div  id="overflow" style="overflow-x: scroll;" >
        ...
        if(isset($_POST['connection'])){
            ...
            if(empty($_POST['todate']) or empty($_POST['fromdate'])){
              $query = "SELECT cust_current_logs.*, connections.name FROM cust_current_logs ,connections WHERE cust_current_logs.cid = connections.cid and cust_current_logs.cid = '".$id."' order by cust_current_logs.id desc limit 1000";
              ?>
            ...
            else
            {
             ...
             $query = "select cust_current_logs.*,connections.name from cust_current_logs,connections WHERE  cust_current_logs.cid = '".$id."'  and cust_current_logs.cid = connections.cid and cust_current_logs.Datetime BETWEEN  '".$fromdate."' AND '".$todate."' order by id desc limit 500";
              ?>
          	...
          	$query= "select cust_current_logs.*,connections.name from cust_current_logs,connections WHERE cust_current_logs.cid = connections.cid and ".$slot2." and cust_current_logs.datetime >NOW()-INTERVAL 1 DAY order by cust_current_logs.id desc limit 1000";
         }
