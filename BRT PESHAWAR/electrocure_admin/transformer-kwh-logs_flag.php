       ...
       <?php
          ...
          if(isset($_POST['transformer'])){
            ...
            if(empty($_POST['todate']) or empty($_POST['fromdate'])){
              $query = "select tr_kwh_logs.*, transformer.name, transformer.kva_capacity from tr_kwh_logs, transformer WHERE tr_kwh_logs.trid = transformer.trid and tr_kwh_logs.trid = '".$id."' order by tr_kwh_logs.datetime desc limit 1500";
              ?>
            ...
            else
            {
              ...
              $query = "select tr_kwh_logs.*, transformer.name, transformer.kva_capacity from tr_kwh_logs, transformer WHERE tr_kwh_logs.trid = transformer.trid and tr_kwh_logs.trid = '".$id."' AND tr_kwh_logs.datetime BETWEEN '".$fromdate."' AND '".$todate."' order by tr_kwh_logs.datetime desc";
              ?>
            ...
            foreach ($result as $row) {
            	array_push($values, array($row['id'],$row['trid'], $row['name'],$row['peak'], $row['offpeak'],$row['peak_dev'], $row['offpeak_dev'],$row['Datetime'], $row['kva_capacity'] ));	
            }
        ?>  
        ...
        <tr >
          ...
          <th scope="col">Website Units</th>
          <th scope="col">Difference</th>
          
          <th scope="col">Device Units</th>
          <th scope="col">Difference</th>
          <th scope="col">Website Units-Device Units</th>
        ...
        <tr <?php if (($values[$i-1][5]+$values[$i-1][6])-($values[$i][5]+$values[$i][6])>$values[$i][8]) {
          echo "class = 'bg-red'";}?>>
        	<td><?php echo $values[$i][0];  ?></td>	
        	<td><?php echo $values[$i][1];  ?></td>	
        	<td><?php echo $values[$i][2];  ?></td>	
        	<td><?php echo round($values[$i][3]+$values[$i][4],2);  ?></td>	
        	<td><?php echo round(($values[$i-1][3]+$values[$i-1][4])-($values[$i][3]+$values[$i][4]),2);  ?></td>	
        	<td><?php echo round($values[$i][5]+$values[$i][6],2);  ?></td>
        	<td><?php echo round(($values[$i-1][5]+$values[$i-1][6])-($values[$i][5]+$values[$i][6]),2);  ?></td>
        	<td><?php echo round(($values[$i][3]+$values[$i][4])-($values[$i][5]+$values[$i][6]),2);  ?></td>
          
          	<td><?php echo $values[$i][7];  ?></td>	
        ...
        <h4>Reminder!</h4>
        Select Transformer from sidebar!
      </div>
      		<?php
      	}
